...
function CardTemplate(props) {
  const modals = <ModalTemplate card = {props.card} />
  return (
    <div>
        ...
        <div className='uk-card-body' style={{ padding: '20px 10px 10px' }}>
          ...
          <div className='uk-flex uk-flex-between' style={{ fontSize: '10px' }}>
            ...
            <div className='uk-flex uk-flex-right uk-align-middle uk-width-1-2' style={{ marginTop: '0px' }}>
              ...
              <p className="uk-margin-remove" style={{ fontSize: '10px', paddingLeft: '5px' }}>{props.card.user}</p>
            </div>
          </div>
        </div>
        <div className='uk-card-footer' style={{ padding: '15px 10px' }}>
          <button className='uk-button uk-button-default uk-button-small' style={{ outline: 'none' }} uk-toggle={`target: #${props.card.id}`} >Get Recipe</button>
        </div>
      </div>
      {modals}
    </div>
